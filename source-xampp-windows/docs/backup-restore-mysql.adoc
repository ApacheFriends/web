...
NOTE: By default, PHP (and therefore phpMyAdmin) is configured to only accept uploads up to 2 MB in size. If your database is larger than this, you need to modify the PHP configuration file to accept a larger upload size. To do this, edit the _php.ini_ file in the _php\_ subdirectory of your XAMPP installation directory and increase the values of the _post_max_size_ and _upload_max_filesize_ variables. Save your changes and restart Apache for the changes to take effect.
